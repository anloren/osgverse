...
## 追加 3(同日)— 底图"污染"3D 城区:改为运行时高程过滤钩子(v3,最终版)

真机反馈:预烘瓦片版(v2)把城区压到 6m,但引擎还要 ×TileElevationScale(2) → 地表浮在
12m,反而盖住 f2 网格地面(浅蓝色补丁);山坡上 ×2 高程更是全程压住网格。

v3(取代 v2,预烘瓦片与脚本已删):给 osgdb_tms 加通用 `ElevationFilterFunction`
setPluginData 钩子(仿 UrlPathFunction;TileCallback terrarium 解码后逐像素回调,默认
NULL 零影响)。EarthExplorer 注入 hkElevationFilter:城区平原核心=0(低于网格地面→被
网格盖住);都会区(含全部山体)=原始×0.5-2m(抵消夸张≈真实高度、略沉,网格贴山面);
区外原样;双边界 smoothstep 渐变;全层级同一连续函数,构造性无缝。z>15 按 z15 祖先
坐标换算(与 createCustomPath 同规则)。EARTH_HK_FLATDEM=0 关闭。

验证:f2 开=网格连续铺满城区、无亮色补丁;f2 关=海岸不淹、城区平整、山体真实高度;
全球远景回归正常。共享代码改动为纯加法钩子(readerwriter/TileCallback + osgdb_tms)。
