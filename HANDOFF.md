...
## Git 现状（重要）
- `master`：已推送 `origin/master`(**HEAD 0f902837**)，含 **P0 标注层 + P1 GIBS 影像层 + 性能优化 + 橙块/晨昏线修复**；.app 已重打包。Tag v0.8/v0.9/v0.9.1 都在（历史），但**只有 v0.8 是 GitHub Release**——用户政策：release 须本人确认，否则只同步仓库（见记忆 `release-requires-confirmation`）。
...
1. **启动预载整个地球低 LOD 瓦片**（用户建议；数据量小）：启动起后台线程，把 z0~4 全球瓦片(ortho+labels，约 340 块)URL 预拉进磁盘缓存→第一次外推到没去过的区域也秒就绪。注意：磁盘预热≠GPU 常驻，配合已做的 targetMax=1500 才完整。
2. **完整异步层加载**（消除首访顿挫）：`TileCallback::operator()`(update=主线程) 在 `_layersDone=false`/`TileManager::check()` 时同步 `readImage` 阻塞(`TileCallback.cpp:552/565`，异步 `requestImageFile` 被注释)。改后台 thread-pool 加载、就绪帧应用。**高风险(并发)、headless 无法验 UX**——留交互会话谨慎做。
3. **瓦片吞吐并行**：每瓦片在 pager 线程顺序下 5 层(ReaderWriterTMS::createTile)；可并行。
