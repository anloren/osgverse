...
## Git 现状（重要）
- `master`：已推送 `origin/master`(HEAD 617941f1)，含 **P0 标注图层 + P1 GIBS 影像层 + 性能优化**。Tag v0.8/v0.9/v0.9.1 都在（历史），但**只有 v0.8 是 GitHub Release**——用户政策：release 须本人确认，否则只同步仓库（见记忆 `release-requires-confirmation`）。
- 设计 spec：`docs/superpowers/specs/2026-06-18-earth-overlay-layers-design.md`（整体路线）。
- **P0（标注层）+ P1（GIBS 影像层）已合并 master**：底图 `lyrs=s`；标注 `lyrs=h`(USER,texUnit2)；GIBS MODIS 真彩(OVERLAY,texUnit3,z>9截断,日期=UTC今天-2天)；`LabelOpacity`/`Overlay2Opacity` 门控(大气采样器从 unit3 挪到 unit4)；`LayerManager`+「图层」UI(方案B，组：底图/标注、影像/天气)。计划见 `docs/superpowers/plans/2026-06-18-earth-overlay-p0-labels.md`、`...-p1-gibs-clouds.md`。
- **性能优化已合并 master**（计划/结论 `docs/superpowers/plans/2026-06-19-earth-perf.md`）：
  - ① **求交优化**(99d3f223)：`EarthManipulator::calcIntersectPoint` 改解析 ray-椭球求交，去掉每次鼠标的全场景 `IntersectionVisitor` 遍历 → 鼠标顺滑。**旋转手感待用户交互确认。**
  - ② **elevation z>15 截断**(dfd8fd09)：AWS terrarium 最高 ~z15，深 zoom 的 z16-19 必 404→主线程 DEFERRED 阻塞(近地卡死主因)+刷屏，已消除（实测 404 归零）；深瓦片复用父级 z15 高程不丢地形。
  - **缓存查明本就好**：真缓存在 `loadFileData`(UtilitiesEx.cpp)，per-tile `.tile`、`$HOME/.osgverse_earth_cache`、无限大小；实测 8537 块缓存、重访 0 新增。用户"缓存没用"实为卡顿错觉。
  - **未做（留交互会话/后续）**：完整层加载线程化(后台 thread pool，高风险、headless 无法验 UX)、瓦片吞吐并行、中国 GCJ-02 偏移(仅中国、近似、风险高)。
- **已知取舍/限制**：GIBS 默认关时仍常驻下载(P1.1 改按需)；无头截图云图加载时 app ~300 帧 shutdown(`EARTH_AUTOCAP`≤280；交互不受影响)；`gmtime_r` 仅 POSIX(Windows 需适配，既有模式)。
- **下一步**：用户交互验证性能改善 → P1.1 GIBS 按需加载 → P2 RainViewer 雷达 → 点要素(USGS/OpenSky，独立 billboard 子图+后台线程)。
