...
6. **低空画面发暗/发蓝(2026-06-21 发现)**：高度下降时白天地表越来越暗发蓝、近地"像黑夜"。**已坐实根因**：大气/曝光管线——高空靠 atmosphere inscatter 撑亮、低空 inscatter 消失只剩偏低的 `HdrExposure=0.25`+大气 pass 蓝调 → 塌暗。**决定性排除**：太阳正头顶(cTheta≈1)600km 仍暗 → **与太阳角/地形法线/ocean mask 无关**;real-time sun 计算正确(北美确为白天);所有 shader = git HEAD 原版(非任何改动引入)。修法属**散射/大气管线级**(雷区,勿反手改):低空随高度自适应曝光、或低空衰减 inscatter 同时补偿地表亮度。当下缓解=拉「曝光 Exposure」(注:「大气强度」滑块实为 `GlobalOpaque` 全局透明度,名不副实)。诊断用 `EARTH_SUN_AZEL=az,el` 钩子。详见记忆 [[earth-low-altitude-dark-atmosphere]]。
