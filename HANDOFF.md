...
- **深 zoom 高程已回退为「平地」求稳定（2026-06-19，最终结论）**：本 session 给深瓦片(z>15)加真实高程（继承→一步到位祖先采样）的尝试**全部放弃**。真因：深瓦片有了高程后，相机在高地形(如 Easter Island ~500m 峰、昆明 1890m)会**穿模**——相机钻到地表之下、边缘观察地形→拉成**尖刺扇**(收敛到一点)。这是"虚线射向太空/尖刺/扇形"的真相(不是几何 bug，是相机穿地)。要支持深 3D 地形必须有**相机-地形碰撞 + 动态近远平面**子系统(逐帧地形 clamp 试过会抖→闪烁；近远平面 callback 会洗白大气)，**headless 无法验证、超出本轮能力，留作单独立项**。
  - **当前状态(稳定)**：`createCustomPath` 对 z>15 ELEVATION 返回 ""→深瓦片建成**海平面平地**(原始行为)。相机(≥150m 椭球高)恒在其上→**不穿模、无尖刺扇**。代价：深 zoom 无 3D 起伏(影像仍全分辨率)，高海拔地区有"地形下沉"的轻微视差(用户最初报的那个，比尖刺扇轻得多)。
  - 残留 dead code：`createTileGeometry` 的 `elevScaleBias` 参数 + `createTile` 里的计算 + `updateLayerData` 去掉 ELEVATION 继承——z>15 elevImage 为 NULL 时这些不生效，无害；要彻底清可回退 TileCallback/ReaderWriterTMS 的相应改动。
  - **保留的好改进**(与此无关)：预热、5层并行、缓存原子写、reader-writer 锁、海洋遮罩继承、最小距离 150m。
