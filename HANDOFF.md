...
## Git 现状（重要）
- `master`：已推送到 `origin/master`。标签/Release：**v0.8**、**v0.9**、**v0.9.1**（latest，极地去截断空洞已撤回、恢复星状）。稳定基线 = v0.9.1。
- 当前分支 **`feature/earth-overlay-layers`**（领先 master 若干提交，**未合并、未推送**）：图层叠加系统。
  - 设计 spec：`docs/superpowers/specs/2026-06-18-earth-overlay-layers-design.md`
  - P0 实现计划：`docs/superpowers/plans/2026-06-18-earth-overlay-p0-labels.md`
- **P0 已完成**（subagent 驱动，逐任务 spec+质量双审）：
  - 底图 Google `lyrs=y`→`lyrs=s`（纯卫星）；标注 `lyrs=h` 进 USER/ExtraLayer 层（texUnit2/UvOffset3），TMS 插件补了 USER 通路。
  - 着色器 `scattering_globe.frag.glsl` 加 `LabelOpacity` uniform（commonUniforms，默认 1.0，clamp）门控标注合成。
  - 最小 `LayerManager`（`applications/earth_explorer/LayerManager.h`）+「图层 Layers」UI 折叠栏（方案 B 分类折叠，开关+透明度）。
  - 实证：贴地/300km 北京 标注开→可见、关→纯净卫星；全球无回归；.app 已重打包验证一致。
- **下一步 P1+**：按 spec 路线接 GIBS 云图→雷达→地震→航班→…（免 key 优先；点要素走独立 billboard 子图 + 后台拉取线程）。
