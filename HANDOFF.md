...
- **⚠️ 深 zoom 几何伪影未解决（2026-06-19，重要）**：用户报告深 zoom 有**尖刺/虚线射向太空 + 闪烁细线 + 移动时变化的三角黑洞 + 道路标记层变形**。试过 3 次盲修都没好：(1) NaN 高程钳制、(2) 高程继承时序竞态→改"一步到位"祖先采样、(3) 都没消除。**关键：headless 无法复现**——z19 俯视 + z12 斜视截图都干净，伪影是稀疏/斜视/移动加载时的瞬态。**已回退本 session 加的「逐帧地形防穿 clamp」**（`EarthManipulator::clampToTerrain`，最可能造成移动时相机抖动→z-fighting 闪烁 + LOD 振荡→瞬态黑洞），看是否消除闪烁/黑洞。**待用户验证**。若仍在：要么继续回退（高程一步到位 fabf2747 → 回到深瓦片平地的稳定态），要么需用户给精确复现条件 / 交互式调试（headless 真的够不到）。一步到位高程本身我验证是干净的（cutoff 临时降 z>5 时 z6-8 山区干净）。
