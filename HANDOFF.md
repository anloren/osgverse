...
## Git 现状（重要）
- `master`：已推送 `origin/master`，含 **P0 标注层 + P1 GIBS 影像层 + 性能优化 + 橙块/晨昏线修复 + 启动低-LOD 预热(backlog #1) + 瓦片 5 层并行加载(backlog #3)**；P0/P1/perf 版的 .app 已重打包，**#1 预热 + #3 并行 这两个 feature 的 .app 尚未重打包**（双击版要生效需 `packaging/package_macos.sh` 重打包）。Tag v0.8/v0.9/v0.9.1 都在（历史），但**只有 v0.8 是 GitHub Release**——用户政策：release 须本人确认，否则只同步仓库（见记忆 `release-requires-confirmation`）。
